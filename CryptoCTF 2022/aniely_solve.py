...
#key == passphrase ^ flag
#enc == passphrase ^ AS(key) ^ rand

#key ^ enc == flag ^ AS(key) ^ rand

i = 0
stream = b''
for _ in aniely_stream(key):
	if i == 32:
		break

	stream += long_to_bytes(_)

	i += 1



side1 = xor(xor(key,enc), stream)

#flag = side1 ^ rand

rand = long_to_bytes(side1[0] ^ ord('C')) + long_to_bytes(side1[1] ^ ord('C'))

print(xor(side1, rand).decode())
